<template>
  <view class="full-screen flex-column flex-center bg-light">
    <view v-if="!writing" class="fill-parent">
      <view class="w-100vw h-90vh flex-column flex-center">
        <image :src="picture" @click="writing = true" :style="canvasStyle"></image>
      </view>
      <view
        class="pos-fixed bottom-0p bg-white w-100vw flex-row align-center justify-around pdtb-5"
      >
        <view
    ...
    <view v-else class="full-screen">
      ...
      <view class="w-100vw h-25vh pos-fixed bottom-0p bg-white pdtb-5 flex-column justify-between">
        <view
          v-if="optIndex == 0"
          class="flex-row align-center justify-around mglr-50 pdb-5"
          style="height: 9vh"
        >
          <view v-for="(item, idx) in shapes" :key="idx" @click="updateShape(item.shape, idx)">
            <view
              v-if="item.shape == 'rect' || item.shape == 'circle'"
              style="width: 18px; height: 18px"
              :class="[
                item.active ? 'bg-blue' : 'bg-u-main',
                { round: item.shape == 'circle', 'radius-2': item.shape == 'rect' },
              ]"
            ></view>
            <yl-icon
              v-else
              :icon="item.icon"
              :size="item.size || 'size-22'"
              :color="item.active ? 'text-blue' : 'text-black'"
            ></yl-icon>
          </view>
        </view>
        <view
        ...
        <view
          v-if="optIndex == 3"
          class="flex-row align-center justify-around mglr-30"
          style="height: 9vh"
        ></view>
        <view class="flex-row align-center justify-around mglr-20" style="height: 8vh">
          <view
            v-for="(item, idx) in writingOptions"
            :key="item.label"
            class="flex-column flex-center pd-5"
            @click="selectWritingOption(idx)"
          >
            <view
              class="square-25p flex-row flex-center"
              :class="{ 'solid border-blue round': optIndex == 1 && optIndex == idx }"
            >
              <yl-icon
                :icon="item.icon"
                :size="optIndex == 1 && optIndex == idx ? 'size-18 lh-1' : 'size-22'"
                :color="idx == 1 ? lineColor : optIndex == idx ? 'text-blue' : item.color"
              ></yl-icon>
            </view>
            <text
              class="text-sm mgt-2"
              :class="[{ 'text-blue': optIndex == idx }, optIndex != idx ? item.color : '']"
            >
              {{ item.label }}
            </text>
          </view>
        ...
        <view
          class="flex-row align-center justify-between padding-lr pdtb-5 mgt-15 mgb-10"
          style="height: 8vh"
        >
          <yl-icon
            icon="cuicon-close"
...
export default {
  data() {
    return {
      ...
      sizes: [
        // 画笔粗细
        { thickness: 2, active: false },
        { thickness: 4, active: true },
  ...
  onLoad() {
    ...
    this.userInfo = userStore.userInfo // 组件加载时获取用户信息
  },
  computed: {
    // 使用计算属性来获取用户信息
    userInfoComputed() {
      const userStore = useUserStore()
      return userStore.userInfo // 直接返回 store 中的 userInfo
    },
    /**
     * 涂鸦操作项
     */
    writingOptions() {
      const { curStep, len } = this.stepInfo
      return [
        {
          label: '涂鸦',
          icon: 'cuicon-write',
  ...
  methods: {
    ...
    choseImage() {
      uni
        ...
        .then((res) => {
          console.log(res)
          const picture = res.tempFiles[0].path
          console.log('res.tempFiles[0]', res.tempFiles[0].path)
     ...
     */
    savePicture() {
      // console.log('username', this.userInfo.username) // 使用计算属性
      // this.$refs.graffiti.saveCanvas(this.userInfo.username)
      this.$refs.graffiti.saveCanvas()
    },
  },
}
</script>
